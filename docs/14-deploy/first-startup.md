...
ES_INDEX_ALIAS=chunks_search
ES_INDEX_SCHEMA_VERSION=2
...
# 向量 Worker 自动声明 documind.embedding.pending、documind.dlx
# 和 documind.embedding.dead。
...
- `EMBED_DIM` 必须等于模型输出维度；系统会校验物理索引 mapping。
- Embedding 模型维度变化时不能复用旧索引，需要创建新索引并重建文档向量。
- 当前 mapping 使用 Elasticsearch 内置 `cjk` analyzer，无需额外安装 IK。
...
RabbitMQ 当前承载向量任务通知；PostgreSQL `vector_jobs` 是任务权威状态，RabbitMQ 故障时由数据库轮询补偿。解析/OCR 暂时仍由应用进程内任务执行。

```env
RABBITMQ_URL=amqp://USER:PASSWORD@HOST:5672/%2f
# 自动声明：documind.embedding.pending
# 死信 exchange/queue：documind.dlx / documind.embedding.dead
```

要求：

- 启动时声明 exchange、queue 和 binding，声明操作必须幂等。
- 向量 Worker 使用 prefetch=1，并通过数据库租约防止重复领取。
- 失败任务指数退避，超过最大次数后进入 dead-letter 队列。
- RabbitMQ 消息只含 job ID，不取代 PostgreSQL 中的任务状态。
...
EMBED_MODEL=text-embedding-v3
EMBED_DIM=1024
EMBED_BASE_URL=https://provider.example.com/v1
EMBED_API_KEY=change-me
EMBED_BATCH_SIZE=10
EMBED_RETRY_MAX=3
```
...
- `EMBED_DIM` 必须和模型输出及 Elasticsearch dense vector mapping 一致。
- 模型、维度或 `ES_INDEX_SCHEMA_VERSION` 变化时使用新物理索引并原子切换别名。
...
- 索引 mapping 与 `EMBED_DIM` 不一致时，Worker 拒绝写入并通过版本化重建恢复。
