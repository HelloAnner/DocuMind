下面是基于你上传的 `docs.zip` 和当前主流技术选型整理的系统优化建议。核心判断是：**你现在体验差的根因不是单纯“前端组件不好”，而是定位链路设计错了**。如果继续让前端拿 `quote` 去正文里 `indexOf`，即使换再好的 PDF/Office 组件，也会出现定位漂移、重复引用、表格定位不准、Word/PPT 页码不稳定等问题。
...
5. 文档原文 URL 不暴露 MinIO 内部地址；当前已支持 `/api/files/{doc_id}/preview-url` 返回带 `preview_token` 的短期 API 代理 URL，通过 `/api/files/{doc_id}/preview/*` 输出预览内容。
...
当前代码/服务器对照总账见：[文档与代码实现差距总账](implementation-gap-analysis.md)。截至 2026-06-28，P0 主链路已基本完成并部署，P1/P2/P3/P4 仍有不同程度差距：引用还需要 claim 级强校验，Office/PDF 还需要精确 bbox 映射，OCR 已有首版但还不是独立 Worker，RabbitMQ 仍未承载完整任务编排。

| 阶段 | 当前状态 | 继续补齐重点 |
|---|---|---|
| 阶段 0 | 已完成主要文档共识，本文档和差距总账统一以 SourceAnchor 为硬契约 | 后续文档必须区分“已部署能力”和“目标架构” |
| P0 | 已部署：SourceAnchor、chunk anchor、ES anchor、CitationOutput.anchor、FileView bbox/char_range 主链路 | PDF text-run 精确 bbox 仍需增强，保持回归验收 |
| P1 | 部分完成：已有 CitationResolver、基础去重和 location_status | 补 claim extractor、数字/日期/实体强校验、引用快照全路径回看 |
| P2 | 部分完成：DOCX/PPTX 可转 PDF 预览，TXT/MD char_range 可高亮 | 补 DOCX/PPTX 结构节点到 preview bbox 映射、表格 cell-level anchor |
| P3 | 部分完成：OCR 首版、golden eval 首版、ES hybrid search 已可用 | 补独立 Python/Document Intelligence Worker、自动 OCR 投递、多粒度 chunk、评估门禁 |
| P4 | 部分完成：健康检查、日志、只读后台统计、部署脚本可用 | 补 RabbitMQ DLX/重试、OTel/metrics/告警、签名 URL、运维操作闭环 |
...
| 0.2 | 更新所有相关设计文档 | `docs/tech.md`、`docs/1-document-parsing/*.md`、`docs/3-chunking/chunking.md`、`docs/9-answer-generation/*.md`、`docs/frontend/file-view.md` | 文档中禁止把“前端用 quote indexOf 定位”写成当前实现方案；保留反例说明时必须明确它是错误路线 |
...
| 序号 | 状态 | 任务 | 交付物 | 验收标准 |
| --- | --- | --- | --- | --- |
| P0.1 | 已完成主链路 | 新增 `document_source_anchors` 表与 chunk anchor 关联 | SQL migration + Rust model | 表结构符合 `docs/1-document-parsing/storage-model.md` |
| P0.2 | 部分完成 | PDF Parser 输出 bbox anchor | `ParsedDocument.anchors` + `document_source_anchors` 数据 | 当前为段落级近似 bbox；text-run/word 级 bbox 仍待增强 |
| P0.3 | 已完成主链路 | `document_blocks` 绑定 `anchor_ids` | block 写入时附带 anchor 关联 | paragraph/table block 可回到 anchor |
| P0.4 | 已完成主链路 | Chunk 携带 `anchor_ids` + `primary_anchor_id` | `Chunk` struct + DB + ES 索引更新 | ES `chunks` 索引有 anchor 字段 |
| P0.5 | 已完成主链路 | 检索返回 `anchor_refs` | `RetrievedChunk` 输出更新 | 检索结果可携带 primary anchor |
| P0.6 | 已完成主链路 | API/SSE 返回 `CitationOutput.anchor` | answer generation 输出更新 | citation 包含 anchor + location_status |
| P0.7 | 已完成主链路 | 前端实现 PDF.js FileView | `apps/web/components/chat/document-viewer` | 浏览器 smoke 已验证 canvas + bbox overlay |
| P0.8 | 已完成主链路 | 禁止前端 `indexOf(quote)` 主定位 | 代码审查 + 删除相关逻辑 | 主链路按 anchor 渲染 |
| P0.9 | 已完成首版 | 服务器端到端验证 | `make deploy` + 浏览器验收 | OCR/PDF citation 浏览器 FileView smoke 已通过；多样本 PDF 误差评估仍待扩展 |
...
| P4.3 | 文件预览签名 URL | `/api/files/{doc_id}/preview-url` | 已完成 API 代理签名版：返回带 `preview_token` 的短期 manifest/content/page PDF URL；S3 presigned 直连仍可作为后续增强 |
