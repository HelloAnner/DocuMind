...
> DocuMind 的用户、租户、角色设计对齐 Northline：同一套多租户身份模型、同一套 RBAC/资源范围控制，只是业务资源从“数据源/语义层”替换为“知识库/文档/检索配置”。本文先定义目标态，当前阶段可继续保留免登录入口，后续按本文补齐认证和角色路由。
...
- 现阶段按项目约束仍可不做登录页，根路径直接进入 DocuMind 工作台。
- 在实现认证前，可用开发态 `mock_current_user`、请求头或本地配置模拟三类角色，先把页面和接口边界搭起来。
- 一旦接入正式认证，根路径 `/` 改为角色路由入口：`super_admin` 进入超级管理员页面，`tenant_admin` 进入当前管理后台，`end_user` 进入普通用户问答页。
