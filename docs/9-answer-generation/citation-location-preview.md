...
2026-06-28 当前代码已经完成主链路切换：citation 输出包含 `anchor`，前端 FileView 使用 `bbox` 或 `char_range` 定位，不再把 `quote` 的全文搜索作为主定位方式。下面表格保留目标差距，但口径从“旧实现 vs 目标实现”更新为“当前实现状态 vs 剩余差距”。

| 当前实现状态 | 剩余目标差距 |
|---|---|
| `CitationOutput` 已返回 `anchor`、`location_status`、`char_range`/`bbox`，`quote` 仅用于卡片摘要 | `quote` 需要进一步压缩为 selected anchor 摘要，而不是直接沿用较长 chunk 内容 |
| CitationResolver 已优先使用 primary anchor 并做基础去重 | 仍需 claim extractor、数字/日期/实体强校验和 claim-resolved evidence 选择 |
| 前端 FileView 已使用 `anchor` 高亮，TXT/MD 用 `char_range`，PDF/Office PDF 用 bbox/页码 | PDF bbox 仍是段落近似；DOCX/PPTX 还缺少结构节点到预览 bbox 的精确映射 |
| ES chunk 已带 anchor 字段，检索结果可恢复 primary anchor | 多 anchor 合并、表格 cell range、失效 parse_job 迁移仍需加强 |
| 右侧主视图已打开原文/预览文件，不再以 chunk 拼接文本作为 citation 主视图 | 管理详情抽屉仍保留 parsed text preview，用于文档管理，不应混同为 citation FileView |
| `conversation_citation_snapshots` migration 已存在 | 需要继续验证历史会话回看是否全部固定到当时 parse_job 和 citation snapshot |

## 16. 推荐实施顺序

1. 已完成：扩展解析输出，为 PDF / DOCX / PPTX / MD / TXT 生成基础 `SourceAnchor`。
2. 已完成：扩展 chunk，保存 `anchor_ids` 和 `primary_anchor_id`，并写入 ES anchor 字段。
3. 已完成：扩展检索返回，`RetrievedChunk` 可携带 primary anchor。
4. 已完成主链路：API/SSE citation contract 返回 `anchor` 和 `location_status`。
5. 已完成主链路：右侧预览接入 FileView，不再使用解析文本 `indexOf` 做主定位。
6. 待加强：CitationResolver 从答案 claim 和 evidence 生成 citation，补 claim extractor、数值/日期/实体校验、复杂去重合并。
7. 待加强：PDF text-run bbox、DOCX paragraph/table cell bbox、PPTX shape/table cell bbox、表格 cell range。
8. 待固化：增加可重复验收用例，覆盖 PDF bbox、Word 段落、PPT shape、表格 cell、chunk overlap 去重、历史 snapshot 回看。
