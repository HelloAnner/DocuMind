...
当前实现状态：服务器已部署 RabbitMQ 并通过 `/api/health` 检查，但解析/OCR/embedding 仍主要由 Rust API 内的进程内异步任务驱动；应用启动时会把上次进程中断的 in-flight 文档任务标记为明确失败或低置信状态，避免永久卡死。下面的队列拓扑仍是生产目标，尚未完整落地为 worker、自动重试和死信链路。
