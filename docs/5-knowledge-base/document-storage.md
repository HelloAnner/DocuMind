...
基于 `ssh documind` 的真实环境：

- `/api/health` 显示 `object_storage=true`，MinIO 容器 `documind-minio` 已运行。
- PostgreSQL 当前在 `documind_dev` 数据库、`documind` schema 下运行。
- `documents.storage_key` 当前已经按 `tenants/{tenant_id}/knowledge-bases/{kb_id}/documents/{doc_id}/original/{sha}.{type}` 生成。
- 当前数据未发现范围错配：documents/kb、chunks/doc、embeddings/chunk、anchors/doc 的 tenant/kb/doc scope mismatch 均为 0。
- 当前 15 张核心表已有 `tenant_id`：`audit_log`、`chunk_embeddings`、`chunks`、`cleaned_blocks`、`conversation_messages`、`conversation_sessions`、`document_blocks`、`document_parse_jobs`、`document_source_anchors`、`document_table_cells`、`document_tables`、`documents`、`knowledge_base`、`knowledge_base_acl`、`tenant_member`。
- `conversation_citations`、`conversation_citation_snapshots` 当前主要通过 `doc_id`、`chunk_id` 或 message/session 关系追溯租户；后续如果要做独立审计查询，应补显式 `tenant_id` 或物化 scope 字段。
...
对象存储 key 是隔离的最后一道防线，不是唯一防线。数据库层必须同步保证 scope 一致：

- `knowledge_base.id` 与 `knowledge_base.tenant_id` 定义知识库归属。
- `documents.tenant_id` 与 `documents.kb_id` 必须匹配同一个知识库，不允许出现文档 tenant 与知识库 tenant 不一致。
- `document_parse_jobs`、`document_blocks`、`document_tables`、`document_table_cells`、`cleaned_blocks`、`chunks`、`chunk_embeddings`、`document_source_anchors` 必须继承文档的 `tenant_id/kb_id/doc_id`。
- 所有文档下载、预览、解析重试、删除和移动接口必须先查数据库 scope，再访问对象存储。
- 后续迁移应补复合唯一键和复合外键，例如 `knowledge_base(tenant_id,id)`、`documents(tenant_id,kb_id,id)`，避免只靠单列外键和应用代码维护一致性。
