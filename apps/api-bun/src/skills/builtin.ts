...
const definitions = [
  {
    ...
    name: 'cnpc-word',
    display_name: '中国石油 Word 文档生成与修改',
    description: '根据用户要求生成结构化 DOCX 文档，或在既有 DOCX 文档末尾追加内容，结果保存为当前用户文件。',
    ...
    modifyInput: '{"source":"既有.docx","sections":[{"heading":"补充章节","paragraphs":["补充正文"]}]}',
    modifyHint: '修改既有文档时保留 source，脚本在既有文档末尾追加章节后写回；输出路径写成与 source 相同的相对路径即为原地修改，写入其它路径则是另存为新文件。',
    output: 'result.docx',
  },
  {
    id: '00000000-0000-0000-0000-00000000c102',
    name: 'cnpc-excel',
    display_name: '中国石油 Excel 表格生成与修改',
    description: '根据用户给出的数据生成 XLSX 工作簿，或在既有工作簿上追加、覆盖工作表内容，结果保存为当前用户文件。',
    scriptPath: 'scripts/cnpc-excel.py',
    imagePath: '/opt/cnpc-skills/cnpc-excel.py',
    script: excelScript,
    input: '{"sheets":[{"name":"数据","rows":[["项目","数值"],["示例",1]]}]}',
    modifyInput: '{"source":"既有.xlsx","sheets":[{"name":"数据","mode":"append","rows":[["库存",7]]}]}',
    modifyHint: '修改既有工作簿时保留 source，并用 sheets[].mode 指定 append（追加行，默认）或 replace（覆盖该表内容）；输出路径写成与 source 相同的相对路径即为原地修改，写入其它路径则是另存为新文件。',
    output: 'result.xlsx',
  },
  {
    id: '00000000-0000-0000-0000-00000000c103',
    name: 'cnpc-ppt',
    display_name: '中国石油 PowerPoint 生成与修改',
    description: '根据用户要求生成 PPTX 演示文稿，或在既有演示文稿末尾追加幻灯片，结果保存为当前用户文件。',
    scriptPath: 'scripts/cnpc-ppt.py',
    imagePath: '/opt/cnpc-skills/cnpc-ppt.py',
    script: pptScript,
    input: '{"slides":[{"title":"标题","bullets":["要点一","要点二"]}]}',
    modifyInput: '{"source":"既有.pptx","slides":[{"title":"标题","bullets":["要点一"]}]}',
    modifyHint: '修改既有演示文稿时保留 source，脚本在既有文件末尾追加幻灯片后写回；输出路径写成与 source 相同的相对路径即为原地修改，写入其它路径则是另存为新文件。',
    output: 'result.pptx',
  },
] as const;
...
export function builtinSkill(idOrName: string): SkillRecord | null {
  ...
  return {
    ...
    content: [
      `# ${skill.display_name}`,
      '',
      '仅在用户明确要求生成或修改对应 Office 文件时使用。先根据用户内容生成 UTF-8 JSON 输入文件，再调用 bash 工具执行内置脚本。',
      `新建文件的输入 JSON 示例：\`${skill.input}\``,
      `修改既有文件的输入 JSON 示例：\`${skill.modifyInput}\``,
      skill.modifyHint,
