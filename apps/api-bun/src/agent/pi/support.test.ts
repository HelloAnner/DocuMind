// pi core 领域支撑测试：对齐旧 kernel_support 的行为基线
import { describe, expect, test } from 'bun:test';
import { boundedHistory, emptyAppliedToolTrace, successfulToolStep } from './support.ts';
import { nowRfc3339 } from '../../infra/time.ts';

describe('pi core support', () => {
  ...
  test('successful_tool_step_preserves_public_call_details', () => {
    const descriptor = {
      ...
      argumentsValue: { query: 'DocuMind 架构' },
    };
    const publicResult = {
      matches: [{ document: '架构说明', content: 'Bun API 与 Next.js 前端' }],
    };
    ...
    expect(step.tool_calls![0]!.arguments).toEqual(descriptor.argumentsValue);
