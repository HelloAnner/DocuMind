...
async fn list_knowledge_bases(
    ...
    if let Some(pool) = &state.db_pool {
        let rows = sqlx::query_as::<_, (Uuid, Uuid, String, Option<String>, String, Vec<String>, chrono::DateTime<chrono::Utc>)>(
        ...
        let summaries = rows.into_iter().map(|(id, tenant_id, name, description, status, tags, updated_at)| {
    ...
    Ok(Json(vec![
        KnowledgeBaseSummary {
            id: state.config.default_kb_ids.get(0).copied().unwrap_or_else(Uuid::nil),
