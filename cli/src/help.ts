...
const RUN_HELP = `用法: documind run <scenario.json|-> [--file-id ID] [--conversation ID] [--json] [--output report.json]\n\n` +
  ...
  `文件断言支持 expect.files_min、expect.file_extensions、expect.file_ids（断言这些文件 ID 出现在本轮 response.files）与 expect.input_files_modified（断言至少一个输入文件被原地写回同一 ID；指定 expect.file_extensions 时被写回的输入文件本身必须命中该扩展名，同轮新建的中间文件不参与判定）。\n` +
